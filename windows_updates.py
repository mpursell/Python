...
def runQuery():
	...
	with open("c:\updates.txt", 'rb') as inputFile:
		...
		#return the values for searchHTML() to use
		return(convertedFile, urlSearch, updateIDSearch)
...
def main():
	...
	#If yes, run the functions to collate updates
	if question == "y":
		
		runQuery()
		#use the results returned by runQuery in searchHTML
		searchHTML(runQuery())
