# Script to update all the ts.xml files within %DeploymentShare%\Control\<Name>\ with the later versions of the OS captures
# Mike Pursell 2014

## Script flow ##
# -> Copies the current Control directory to ControlBAK
# --> If that's successful, use the Control\OperatingSystems.xml to create a dictionary of image names and GUIDs
# ----> Use the dictionary to look up the GUIDs for the given image names and slot them into specific indices into 2 lists, old and new OSs
# -----> Find and replace all instances of old list items with the corresponding new list items - eg oldlist[0] -> oldlist[0]
...
# Enter our Deploymentshare\Control folder path
...
#function to copy the current Control folder to a backup
def backup():
...
#function to replace the OS GUIDs with the new ones in each of the task sequences, takes the os_dict returned by osGuids() and the task sequence directory list from listTsDirs arguments
def osReplace(operating_system_dict, ts_dir_list):
    ...
    for ts_directory in ts_dir_list:
        ...
        with open(folder, 'r') as task_sequence: #open and read in the xml file as a string so we can call .replace()
            ...
            index_counter = 0 #since we explictly set the indices for the lists, we can use slice and counters

            for item in old_guids_list:
                task_sequence = task_sequence.replace(old_guids_list[index_counter], new_guids_list[index_counter]) # do a like for like - old_guids[0] for new_guids1[0] etc...
...
def main():
    ...
    if backup() == None: # make sure the backup is place before we do anything!  If we get a successful return code from backup() carry on.
